...
def address_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from address where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'address' order by addtime desc")
...
def address_count(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT count(*) AS count FROM address {0}".format(where)
        count = 0
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
