...
def aboutus_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if req_dict.get("mima") and req_dict.get("password"):
            if "mima" not  in aboutus.getallcolumn(aboutus,aboutus) :
                del req_dict["mima"]
            if  "password" not  in aboutus.getallcolumn(aboutus,aboutus) :
...
def aboutus_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM aboutus {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def aboutus_count(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT count(*) AS count FROM aboutus {0}".format(where)
        count = 0
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
